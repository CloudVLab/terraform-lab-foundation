...
def ${var.step_name}(handles:, maximum_score:, resources:)
  ...
  # The target_tags variable is an array

  # The any? method directly checks if there's at least one element in the array
  # that matches your condition, stopping as soon as one is found.

  isAvailable = resource_object.target_tags.any? do |tag|
    tag.downcase.include?(fw_target_tag)
  end
