...
FUNCTION_DIR="tf/cf"
FUNCTION_ZIP1="account.zip"
FUNCTION_ZIP2="mostplayed.zip"
FUNCTION_URL="${URL}/${TYPE}/${MODULE}/example/cf/"
...
# Download if the file does not exist
if [ ! -f "$FUNCTION_DIR/$FUNCTION_ZIP1" ]; then
  curl -L $FUNCTION_URL/$FUNCTION_ZIP1 -o "$FUNCTION_DIR/$FUNCTION_ZIP1"
fi

# Download if the file does not exist
if [ ! -f "$FUNCTION_DIR/$FUNCTION_ZIP2" ]; then
  curl -L $FUNCTION_URL/$FUNCTION_ZIP2 -o "$FUNCTION_DIR/$FUNCTION_ZIP2"
fi
